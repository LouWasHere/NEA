...
public class DatabaseManagement {
    ...
    public void menu() {
        ...
        while (loop) {
            ...
            do {
                ...
                System.out.println(
                        "1. Add new database information\n2. Delete database information\n3. Exit to main menu");
    ...
    private void addRoad() {
        ...
        try {
            ...
            stmt.executeUpdate("INSERT INTO RoadInfo (RoadName, RoadLength, RoadCurvature, TrafficID) VALUES (\""
                    + roadName + "\", " + roadLength + ", " + roadCurvature + ", " + trafficLevel + ");");
            conn.commit();
    ...
    private void addCorner() {
        ...
        try {
            stmt = conn.createStatement();
            stmt.executeUpdate(
                    "INSERT INTO EdgeInfo (RoadID, CornerID) VALUES (" + firstRoadID + ", " + cornerID + ");");
            conn.commit();
        } catch (SQLException e) {
            System.out.println("Error: " + e);
        }
        ...
        try {
            stmt = conn.createStatement();
            stmt.executeUpdate(
                    "INSERT INTO EdgeInfo (RoadID, CornerID) VALUES (" + secondRoadID + ", " + cornerID + ");");
            conn.commit();
        } catch (SQLException e) {
            System.out.println("Error: " + e);
        }
    ...
    private void addRacer() {
        ...
        do {
            ...
            System.out.println(
                    "Please enter the ID of the vehicle the driver is associated with (insert 0 for no vehicle)");
            while (!scanner.hasNextInt()) {
                System.out.println("That's not a number!");
                scanner.next();
            }
        ...
        try {
            stmt = conn.createStatement();
            stmt.executeUpdate("INSERT INTO RacerInfo (VehicleID, EmergencyNo, RacerForename, RacerSurname) VALUES ("
                    + vehicleID + ", \"" + contactNumber + "\", \"" + forename + "\", \"" + surname + "\");");
            conn.commit();
        } catch (SQLException e) {
            System.out.println("Error: " + e);
        }
    ...
    private void deleteData() {
        ...
        switch (userSelection) {
            case 1:
                System.out.println(
                        "Please note that deleting a road will delete any corners that this road was attached to. If you wish to just remove a join\nbetween two roads please use \"Delete Corner\".");
                try {
                    ResultSet rs = null;
                    stmt = conn.createStatement();
                    rs = stmt.executeQuery("SELECT RoadID, RoadName FROM RoadInfo;");
                ...
                try {
                    ...
                    System.out
                            .println("Error: You probably provided an invalid Road ID. Please exit and try again." + e);
                }
                System.out.println("Road Name: " + RoadName
                        + " will be deleted, along with any corners that attach it to other roads. Do you wish to proceed? Enter Y to confirm.");
                selection = scanner.nextLine();
                if (selection.equals("Y") || selection.equals("y")) {
            ...
            case 2:
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        rs2 = stmt.executeQuery(
                                "SELECT RoadInfo.RoadName AS RoadName FROM EdgeInfo, RoadInfo WHERE EdgeInfo.CornerID="
                                        + cornerID + " AND EdgeInfo.RoadID=RoadInfo.RoadID;");
                ...
                scanner.nextLine();
                System.out.println("Corner " + cornerID
                        + " will be deleted. The roads it connects will remain, so that a corner may be reinstated if desired. Proceed? Enter Y to confirm.");
                selection = scanner.nextLine();
                if (selection.equals("Y") || selection.equals("y")) {
                    System.out.println("Confirmed. Deleting records...");
                    try {
            ...
            case 3:
                ...
                try {
                    ...
                    while (rs.next()) {
                        System.out.println("RacerID: " + rs.getInt("RacerID") + ", Name: "
                                + rs.getString("RacerForename") + ", " + rs.getString("RacerSurname"));
                    }
                } catch (SQLException e) {
                    System.out.println("Error: " + e);
                }
            ...
            case 4:
                System.out.println(
                        "Deleting a vehicle will set all drivers associated with this vehicle to the status of NO VEHICLE.");
                int vehicleID;
                try {
                    ResultSet rs = null;
                    stmt = conn.createStatement();
                    ...
                    while (rs.next()) {
                        System.out.println(
                                "VehicleID: " + rs.getInt("VehicleID") + ", Model: " + rs.getString("VehicleModel"));
                    }
                } catch (SQLException e) {
                    System.out.println("Error: " + e);
                }
